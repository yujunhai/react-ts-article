...
let snowImage = new Image() // 绘制目标，也就是雪花
...
export function init() {
  ...
  // 设置雪花图片路径
  snowImage.src = require('./snow.png')
  // 不断绘制雪花，内部有requestAnimationFrame，内部会自动执行不断回调最后实现动画效果
  loop()
}

export function destory() {
  var canvasHtml = document.querySelectorAll('.snowCanvas')
  for (let i = 0; i < canvasHtml.length; i++) {
    document.body.removeChild(canvasHtml[i])
  }
  ctx.clearRect(0, 0, W, H)
}
